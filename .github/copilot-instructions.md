...
- **ASCII only** - NEVER put Unicode characters in sources (code, comments, UI
  strings, Markdown). Use ASCII equivalents: `>` for arrows, `-` for en/em dashes,
  `...` for ellipsis, straight `"` and `'` for smart quotes. If a glyph is
  genuinely needed at runtime, use an escape (`\uXXXX` in JS/Python, `&#xXXXX;` or
  `&times;` in HTML). The only exception is proper names that legitimately carry
  accents (e.g. contributor names in `CREDITS.md`).
...
- **Structure** - every test has 3 sections:
  ...
  def test_something():
      ...
      ...
  ```
- **Output**: use `DONE` text, never emoji or other non-ASCII characters.
