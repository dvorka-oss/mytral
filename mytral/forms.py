...
class ImportGarminArchiveForm(flask_wtf.FlaskForm):
    """Form for importing activities from a Garmin Connect data archive."""

    data_dir = wtforms.StringField(
        label="Data directory",
        description=(
            "Absolute path to the extracted Garmin Connect archive directory "
            "(the one containing DI_CONNECT/ and customer_data/ subdirectories)."
        ),
        validators=[validators.DataRequired()],
    )
    on_conflict = wtforms.RadioField(
    ...
    submit = wtforms.SubmitField("Import Garmin Archive")
