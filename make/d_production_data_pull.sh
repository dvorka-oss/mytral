...
echo "PULL production data from Git repo & sync BLOBs from the shared drive..."
...
cd ${_SHARED_BLOBS_DIR} || exit 1

ls -d */ | \
while read -r MYTRAL_USR_DIR
do
  echo "  Syncing MyTraL user dir ${MYTRAL_USR_DIR%/} ..."
    FROM_DIR="${_SHARED_BLOBS_DIR}/${MYTRAL_USR_DIR%/}/blobs"
    TO_DIR="${_MYTRAL_DATA_DIR}/${MYTRAL_USR_DIR%/}/blobs"
